...
### 4.7 Post a Project — AI Hiring Assistant
- **Route:** `/client/projects/new`
- **Purpose:** Core AI-assisted project creation flow.
- **Stage 1 — Natural language input:** centered large textarea ("Describe your project in plain English — budget, timeline, what you need built…"), gradient "Generate with AI" button with sparkle icon.
- **Loading state:** pulsing violet sparkle, "Gonka AI is analyzing your project…" (per master §9's simulated Parse Project call).
- **Failure state:** if the simulated call fails, keep the typed description intact, show inline error ("Couldn't generate a structured plan right now") + "Try Again" button, plus a lower-emphasis "Skip AI and fill in manually" link that goes straight to a blank Stage 2 form.
- **Stage 2 — Editable AI-generated form:** violet-tinted banner "AI-generated — review and edit before posting" with the Gonka Request ID in mono text and a "Regenerate" ghost button (re-runs Stage 1). Fields: Project Title, Required Skills (editable chip list), Estimated Budget, Timeline, and a Milestones section built from repeatable **Milestone Finalize Rows** (§3) with the running-total indicator pinned above, "+ Add Milestone" ghost button. Bottom bar: ghost "Save as Draft" (sets status `draft`, routes to §4.3) and gradient "Post Project" — disabled with an inline note if milestone percentages don't sum to 100%.
- **Confirmation state:** on successful post (status → `open`), brief "Project posted ✓" moment, then route into §4.4, landed on the Candidates tab so the Recommended list is already computing.
...
- **Part 1 — Finalize Milestones:** same Milestone Finalize Rows, pre-filled with the project's existing plan, fully editable, running-total indicator pinned above. Heading: "Confirm milestones with [Freelancer Name] before funding escrow." Gradient "Confirm & Continue" disabled until total = exactly 100%.
- **Part 2 — Fund Escrow:** heading "Fund Escrow for [Project Title]." Summary card: freelancer avatar + name, total USDC amount, the now-finalized milestone breakdown (read-only), reassurance copy: "Funds are locked in a Sui smart contract and only released when you approve each milestone — TrustHire never holds your funds." Wallet section: connected wallet chip, network indicator, "Gas fees sponsored by TrustHire" teal badge (cosmetic only in this prototype).
