...
export function WalletConnectButton({ disabled }: { disabled?: boolean }) {
    ...
    if (isDisconnecting) {
        return (
            <div className="flex items-center gap-2 px-4 py-2.5 bg-black/[0.02] dark:bg-[#151622] text-foreground/50 font-medium rounded-xl border border-black/10 dark:border-white/10 w-full justify-center">
        ...
        return (
            <div className="relative w-full" ref={accountMenuRef}>
                <button
                    ...
                    className="w-full cursor-pointer flex items-center justify-between px-4 py-2.5 bg-white/50 dark:bg-[#151622] text-foreground font-medium rounded-xl border border-black/10 dark:border-white/10 hover:border-[#2563EB]/30 dark:hover:border-[#4DA2FF]/30 hover:bg-black/5 dark:hover:bg-white/5 transition-all duration-200"
                >
                ...
                {showAccountMenu && (
                    <div className="absolute left-0 mt-2 w-full bg-white/90 dark:bg-[#151622]/90 border border-black/10 dark:border-white/10 rounded-xl shadow-xl z-50 overflow-hidden backdrop-blur-xl">
                        <div className="p-3 border-b border-black/10 dark:border-white/10">
                            <p className="text-xs text-foreground/50 mb-1">Connected Address</p>
                        ...
                        <div className="p-2">
                            <button
                                ...
                                className="cursor-pointer w-full flex items-center gap-3 px-3 py-2 text-sm text-foreground/80 hover:text-foreground hover:bg-black/5 dark:hover:bg-white/5 rounded-lg transition-colors"
                            >
                                {copied ? <Check className="w-4 h-4 text-[#2DD4BF]" /> : <Copy className="w-4 h-4 text-foreground/50" />}
                                {copied ? "Copied!" : "Copy Address"}
                            </button>
                            <button
                                type="button"
                                onClick={handleDisconnect}
                                className="cursor-pointer w-full flex items-center gap-3 px-3 py-2 text-sm text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-500/10 dark:hover:bg-red-400/10 rounded-lg transition-colors"
                            >
