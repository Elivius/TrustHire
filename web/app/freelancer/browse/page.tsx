  ...
  return (
    <AppShell>
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Filter & Search Bar */}
        <div className="p-4 rounded-2xl border border-white/10 bg-[#151622]/60 backdrop-blur-md space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-xs rounded-xl border border-white/10 bg-white/[0.03] text-white focus:outline-none focus:border-[#7B61FF]"
              />
            </div>
            ...
            <div>
              <select
                ...
                className="w-full px-3 py-2 text-xs rounded-xl border border-white/10 bg-white/[0.03] text-white focus:outline-none"
              >
                <option value="all" className="bg-[#151622]">All Budget Ranges</option>
                <option value="<500" className="bg-[#151622]">&lt; $500 USDC</option>
                <option value="500-2k" className="bg-[#151622]">$500 – $2k USDC</option>
                <option value="2k-10k" className="bg-[#151622]">$2k – $10k USDC</option>
                <option value="10k+" className="bg-[#151622]">$10k+ USDC</option>
              </select>
            </div>
            ...
            <div>
              ...
              >
                {activeTab === "recommended" && (
                  <option value="best_match" className="bg-[#151622]">Sort: Best AI Match</option>
                )}
                <option value="newest" className="bg-[#151622]">Sort: Newest First</option>
                <option value="budget" className="bg-[#151622]">Sort: Highest Budget</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {sortedProjects.map((project) => {
              ...
              return (
                <GlassCard
                  key={project.id}
                  className="p-5 sm:p-6 space-y-4 hover:border-white/20 transition-all group"
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="space-y-1.5 flex-1">
                      <div className="flex items-center gap-2.5 flex-wrap">
                        <Link href={`/project/${project.id}`}>
                          <h3 className="text-base sm:text-lg font-bold text-white group-hover:text-[#4DA2FF] transition-colors">
                            {project.title}
                          </h3>
                        </Link>
                        {activeTab === "recommended" && (
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                      <button
                        ...
                        className="p-2 rounded-xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.08] text-foreground/75 hover:text-white transition-all cursor-pointer"
