...
export const CompletionRatingModal: React.FC<CompletionRatingModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-[#151622] p-6 sm:p-8 shadow-2xl space-y-5 text-center">
        <div className="w-12 h-12 rounded-2xl bg-[#2DD4BF]/20 text-[#2DD4BF] flex items-center justify-center mx-auto">
        ...
        <div className="flex items-center justify-center gap-2 py-2">
          {[1, 2, 3, 4, 5].map((s) => (
            <button
              ...
              type="button"
              onClick={() => setStars(s)}
              className="p-1 text-[#2DD4BF] hover:scale-110 transition-transform cursor-pointer"
            >
              <Star
                className={clsx(
                  "w-8 h-8",
        ...
        <textarea
          ...
          className="w-full p-3 rounded-xl border border-white/10 bg-white/[0.03] text-xs text-white focus:outline-none focus:border-[#2DD4BF] resize-none leading-relaxed"
        />
