...
- **Layout:** 4-step flow, centered glass card, thin progress bar/step dots, consistent with the client onboarding pattern.
  ...
  - **Step 3 — Portfolio:** repeatable rows (title + URL), "+ Add Link" ghost button, optional file/image upload for work samples. Skippable via ghost "Skip for now" link — but copy notes "A portfolio meaningfully improves your Trust Score and match quality."
...
- **Main content, top to bottom:**
  - Greeting header ("Welcome back, [Name]") with Trust Score badge (large teal ring) beside it, small "View Score Breakdown" link underneath → §4.9.
  ...
  - **"AI-Recommended Projects For You"** section — small "Powered by Gonka AI" sparkle label. 3–4 project cards (title, budget, required-skill chips, violet Match Score badge, one-line AI reasoning e.g. "Recommended because your React + Move skills match 92% of requirements"), "View & Apply" → §4.4. "See All Recommendations →" routes to §4.3 with the Recommended tab pre-selected.
